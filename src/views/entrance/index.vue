...
import md5 from 'crypto-js/md5'
  ...
  const handleRouteChange = (entrance) => {
    ...
    if (!entrance.canCheck) {
      createMessage.warning('暂无权限访问!');
      return;
    }
    const menus = unref(menusRef);
    if (entrance.path.includes('/survey/')) {
      if (entrance.path == '/survey/') {
        window.open('/survey' + (entrance.redirect || entrance.path));
      } else {
        console.log('menus', menus);

        if (entrance.path == '/survey/networkDisk') {
          window.open('/survey/survey/networkDisk');
        }else if(entrance.path == '/survey/businessInsight'){
          window.open('survey' + entrance.redirect);
        }
         else {
          //筛选出菜单第一个
          const willingItem = menus.find((item) => {
            if (item.path.includes('/survey/') && item.path != '/survey/') {
              console.log('item.path', item.path);
              return item;
            }
          });
          console.log('willingItem', willingItem);
          const oPath = willingItem.children == undefined ? willingItem.path : willingItem.children[0].path;
          // console.log('willingItem', willingItem, willingItem.path)
          window.open('/survey' + oPath);
        }
      }
    } else {
