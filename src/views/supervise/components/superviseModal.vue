...
<script lang="ts" setup>
  ...
  import { switchSingleChoiceDropdownFormat } from '../../taskTemplateConfig/designerConfig/common';
  ...
  const setResult = (params) => {
    superviseEditView(params).then((res) => {
      ...
      const finalFormData = switchSingleChoiceDropdownFormat(pageSchema.value, data, 'display');
      setTimeout(() => {
        // 表单赋值
        EBuilderref.value!.setData(finalFormData);
      }, 300);
    });
  };
