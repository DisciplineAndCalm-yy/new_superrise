<template>
    <BasicModal @register="register" v-bind="$attrs" title="一键督办" @ok="handleSubmit" @cancel="closeFn" width="90%">
        ...
        <a-space v-if="formData.supDataList.length > 0" :size="size">
            <span>一级指标：{{ formData.supDataList[0].formJsonObj.firstIndicatorName }}</span>
            <span>二级指标：{{ formData.supDataList[0].formJsonObj.secondIndicatorName }}</span>
        </a-space>
        <!-- :row-selection="rowSelection" -->
        <a-table bordered :rowKey="record => record.id" :columns="columns" :data-source="formData.supDataList"
            ...
            <template #bodyCell="{ column, record, index }">
                ...
                <template v-if="column.dataIndex === 'mainContent'">
                    <textarea type="text" v-model="record.formJsonObj.mainContent" placeholder="请输入"
                        class="width-100-percent formPublic" />
                </template>
                <template v-if="column.dataIndex === 'responsibleDepart'">
                    <a-select v-model:value="record.formJsonObj.responsibleDepart" class="width-100-percent" allowClear show-search optionFilterProp="label" :filter-option="filterOption"
                        placeholder="请选择">
                        <a-select-option :value="item.dictCode" v-for="item in orgs" :key="item.dictCode" :label="item.dictName">{{ item.dictName
                            }}</a-select-option>
                    </a-select>
                </template>
...
const [register, { openModal, closeModal, setModalProps }] = useModalInner(async (data) => {
    ...
    exampleObj.formJsonObj.firstIndicatorName = data.level1IndName
    exampleObj.formJsonObj.secondIndicatorName = data.level2IndName
    Object.assign({}, exampleObj)
    formData.supDataList.push(JSON.parse(JSON.stringify(exampleObj)))
})
...
const handleSubmit = (type) => {
    ...
    formData.supDataList.map(item => {
        item.formJsonObj.attachments = item.formJsonObj.fileValue.map(file => file.storageId).join(',')
        return item
    })
