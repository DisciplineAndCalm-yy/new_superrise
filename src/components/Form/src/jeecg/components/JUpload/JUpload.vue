...
<script lang="ts" setup>
  ...
  const props = defineProps({
     ...
     * 是否返回url，
     * true：仅返回url
     * false：返回storageName storageUrl storageSize storageId
     */
  ...
  function parseArrayValue(array) {
    ...
    for (const item of array) {
      ...
      console.log('unitem url', unitem.storageUrl)
      // let url = getFileAccessHttpUrl(item.filePath);
      let url = unitem.storageUrl;
      list.push({
        uid: uidGenerator(),
        name: unitem.storageName,
        url: url,
        status: 'done',
        response: { status: 'history', result: {path: unitem.storageUrl, storageId: unitem.storageId} },
      });
    }
    fileList.value = list;
      ...
      if (info.file.response.success) {
        ...
        fileListTemp = fileListTemp.map((file) => {
          ...
          if (file.response) {
            ...
            let reUrl = file.response.result.path;
        ...
        for (const item of fileListTemp) {
          if (item.status === 'done') {
            let fileJson = {
              ...
              storageUrl: item.response.result.path,
  ...
  function handlePathChange() {
    ...
    //   storageId = storageIdList.join(',');
    // }
    // console.log('storageIdList', storageIdList)
    emitValue(path);
    // emitValue(storageId);
  }
