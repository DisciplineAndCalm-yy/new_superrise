...
<script lang="ts" setup>
  ...
  const props = defineProps({
    modelValue: {
      type: Object as PropType<typeof formState.value>,
      require: true,
      default: () => ({}),
    },
    record: {
  ...
  const getOptions = computed(() => {
    if (props.record.componentProps.completionDeadline === DeadlineTypeEnum.DATE_SELECT) {
      return {
        noticeTimeOptions: props.record.componentProps.noticeTimeOptions,
        noticeCycleOptions: props.record.componentProps.noticeCycleOptions,
      };
    } else {
      return {
        noticeTimeOptions: props.record.componentProps.noticeTimeOptions.filter((i) => i.value === '1'),
        noticeCycleOptions: props.record.componentProps.noticeCycleOptions.filter((i) => i.value === '1'),
      };
    }
  });
  ...
  watch(
    () => props.record.componentProps.completionDeadline,
    (val) => {
      if (val === '0') {
  ...
  watch(
    () => props.modelValue,
    (val) => {
      if (first && Object.keys(val).length && !shallowCompare(val, formState.value)) {
        formState.value = val;
        first = false;
      }
    },
