...
import { multipleTextareaConfig } from '../../components/multiple-textarea';
...
import { multipleInputConfig } from '../../components/multiple-input';
import { richTextEditorConfig } from '../../components/rich-text-editor';
import { sliderConfig } from '../../components/slider';
import { deadlineTypeConfig } from '../../components/deadline-type';
...
const componentTypeConfig = {
  ...
  [ComponentTypeEnum.MULTIPLE_TEXTAREA]: multipleTextareaConfig,
  [ComponentTypeEnum.CHECK_BOX]: checkBoxConfig,
  [ComponentTypeEnum.MULTIPLE_INPUT]: multipleInputConfig,
  [ComponentTypeEnum.RICH_TEXT_EDITOR]: richTextEditorConfig,
  [ComponentTypeEnum.SLIDER_CONFIG]: sliderConfig,
  [ComponentTypeEnum.DEADLINE_TYPE]: deadlineTypeConfig,
};
...
export const generateSchemaByType: GenerateSchemaByType = (config, mode: 'cover' | 'merge') => {
  ...
  if ('rules' in resultConfig.defaultSchema) {
    ...
    resultConfig.defaultSchema.rules[0].message = `该项不能为空`;
  }
...
export const generateSchemaByConfig = (config: SchemaItem[], mode: 'cover' | 'merge') => {
  ...
  console.log('所有注册组件', finalySchemaList);
